...
const DashboardLayout = () => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (provider?.licenses && Array.isArray(provider.licenses)) {
        const types = [
          ...new Set(provider.licenses.map((item) => item.service_type)),
        ];
      ...
      console.error(
        "❌ Error parsing provider types from localStorage:",
        error
      );
    }
  }, []);
  ...
  if (providerTypes.includes("hotel")) {
    ...
    if (hotelId) {
      ...
      console.log(
        "⚠️ Hotel provider but no hotel found - showing ONLY create hotel option"
      );
      menuItems.push({
          ...
          {providerTypes.includes("tour") && (
            <div className="nav-item-wrapper">
              ...
              >
                ...
                <span
                  className={`dropdown-arrow ${isTourMenuOpen ? "open" : ""}`}
                  aria-hidden="true"
                >
                  ▾
                </span>
